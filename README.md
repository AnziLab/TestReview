...
| 기능 | 설명 |
|------|------|
| 📄 채점기준표 자동 추출 | 채점기준표 PDF를 올리면 Gemini AI가 문항·모범답안·채점기준을 자동 인식 |
...
| 기능 | 설명 |
|------|------|
| 🖨 단면·양면 스캔 PDF 업로드 | 1페이지=1학생, 2페이지=1학생 모드 지원 |
| 📑 앞뒤 분리 스캔 | 답안지가 얇아 양면 스캔 어려울 때 앞면/뒷면 PDF를 따로 올리면 자동 인터리브 |
| ⏳ 실시간 진행률 | 학생별 OCR 진행 상황을 프로그레스바로 표시 |
| ✏ 학번/이름 일괄 입력 | 엑셀에서 학번·이름을 복사해 붙여넣으면 행 순서대로 자동 채움 |
| 🔄 학생별 재OCR | 인식 실패한 학생만 콕 집어 PDF 재업로드 후 다시 인식 |
| ✂ 취소선 답안 무시 | 학생이 줄 그어 지운 답안은 인식에서 제외 |

### 채점 & 결과
| 기능 | 설명 |
|------|------|
| 🤖 일괄 채점 | 완성된 기준표로 전체 학생 자동 채점 |
| 🎯 반 선택 채점 | 여러 반 중 원하는 반만 골라 채점 (다른 반 결과는 보존) |
| 📈 채점 진행률 | 문항별 처리 상황을 프로그레스바로 표시 |
| 📥 Excel 다운로드 | 컬럼 선택 가능 — 점수, 채점이유, 학생 답안, 모범답안, 매칭 채점기준, 합계 중 원하는 항목만 |

### 고급
| 기능 | 설명 |
|------|------|
| 🛠 프롬프트 커스터마이징 | OCR/채점/클러스터링/채점기준 추출·생성 등 6개 AI 프롬프트를 사용자별로 통째 편집 가능, 잘못 편집 시 자동 폴백 + "기본값으로 복원" 버튼 |
| 💬 추가 지시사항 | 과목 특성·특별 규칙을 채점/클러스터링 프롬프트 끝에 덧붙이는 가벼운 옵션 |
...
2. 학생 답안 업로드
   └─ 반별 스캔 PDF 업로드 (단면 / 양면 / 앞뒤 분리)
   └─ AI가 답안을 자동 인식 (학번/이름은 표에 직접 입력 또는 엑셀 붙여넣기)
   └─ 인식 실패한 학생은 🔄 버튼으로 재OCR
...
4. 채점
   └─ 완성된 기준표로 전체 또는 선택한 반만 자동 채점
   └─ 진행률 실시간 표시
   └─ 원하는 컬럼만 골라 Excel 다운로드
```
...
- DB 마이그레이션은 앱 재시작 시 자동 적용됩니다 (실행 중에는 SQLite 잠금 때문에 적용 불가)
