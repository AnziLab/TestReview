...
- 반드시 유효한 JSON 객체만 반환하고, JSON 앞뒤에 설명이나 마크다운 코드 블록을 추가하지 않음
- LaTeX의 역슬래시(`\\`)는 JSON 문자열 규칙에 맞게 반드시 이스케이프하여 유효한 JSON을 유지
...
주의사항:
- 반드시 유효한 JSON 객체만 반환하고, JSON 앞뒤에 설명이나 마크다운 코드 블록을 추가하지 않음
- 제공된 모든 답안에 대해 정확히 하나의 결과를 반환하며 누락하거나 중복하지 않음
- answer_id는 학생 답안 목록에 제공된 값을 수정하지 않고 그대로 사용
- score는 0 이상 {max_score} 이하
- matched_criteria_ids는 해당 기준이 없으면 빈 배열 []로 반환
